...
public sealed class DropdownDataService
{
    ...
    public ObservableCollection<DictItemDto> Specifications { get; } = new();
    public ObservableCollection<DictItemDto> Models { get; } = new();
    public ObservableCollection<DictItemDto> Manufacturers { get; } = new();
    public ObservableCollection<DictItemDto> Projects { get; } = new();
    ...
    public void RefreshAll()
    {
        lock (_lock)
        {
            ...
            var specs = db.Queryable<Specification>()
                .OrderBy(s => s.Name)
                .Select(s => new DictItemDto { Id = s.Id, Name = s.Name })
                .ToList();
            RefreshCollection(Specifications, specs);

            var models = db.Queryable<PartModel>()
                .OrderBy(m => m.Name)
                .Select(m => new DictItemDto { Id = m.Id, Name = m.Name })
                .ToList();
            ...
            var projects = db.Queryable<Project>()
                .OrderBy(p => p.Name)
                .Select(p => new DictItemDto { Id = p.Id, Name = p.Name })
                .ToList();
    ...
    private static void RefreshCollection(ObservableCollection<DictItemDto> target, List<DictItemDto> source)
    {
        target.Clear();
